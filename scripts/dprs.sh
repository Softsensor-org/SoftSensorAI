...
# SoftSensorAI Readiness Score (DPRS) - Measure repository maturity
set -euo pipefail
...
# Get repository info
REPO_NAME=$(basename "$(get_git_root)" 2>/dev/null || echo "unknown")
BRANCH=$(git branch --show-current 2>/dev/null || echo "main")
SHA=$(git rev-parse --short HEAD 2>/dev/null || echo "unknown")
TODAY=$(date +%Y-%m-%d)

log "SoftSensorAI Readiness Score (DPRS)"
...
echo "" >> "$OUTPUT_DIR/dprs.md"
echo "---" >> "$OUTPUT_DIR/dprs.md"
echo "*Generated by SoftSensorAI Readiness Score on $TODAY*" >> "$OUTPUT_DIR/dprs.md"

# Display results
log "DPRS Results"
info "Total Score: ${TOTAL_SCORE}/100"
info "Phase Readiness: ${PHASE_READINESS}"
echo "Category Breakdown:"
echo "  Tests: ${TESTS_SCORE}/100"
echo "  Security: ${SECURITY_SCORE}/100"
echo "  Documentation: ${DOCS_SCORE}/100"
