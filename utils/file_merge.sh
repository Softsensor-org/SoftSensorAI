...
# Check if file has SoftSensorAI markers
has_softsensorai_markers() {
  local file="$1"
  grep -q "SoftSensorAI\|SOFTSENSORAI\|Generated by SoftSensorAI\|softsensorai:" "$file" 2>/dev/null
}
...
merge_claude_md() {
  ...
  # Add merge notice
  echo "<!-- SoftSensorAI Merged: $(date +%Y-%m-%d) -->" >> "$merged"
  echo "" >> "$merged"
  ...
  # Add SoftSensorAI standard sections from new file
  echo "## SoftSensorAI Configuration" >> "$merged"
  echo "" >> "$merged"
...
merge_gitignore() {
  ...
  # Add SoftSensorAI section marker if not present
  if ! grep -q "# SoftSensorAI" "$merged"; then
    echo "" >> "$merged"
    echo "# SoftSensorAI additions" >> "$merged"
  fi
...
merge_file() {
  ...
  if [ "$strategy" = "auto" ]; then
    ...
    # If file has SoftSensorAI markers, prefer merge
    if has_softsensorai_markers "$target_file"; then
    ...
    "$STRATEGY_MERGE")
      ...
      case "$filename" in
        ...
        *)
          ...
          echo "" >> "$temp_merged"
          echo "# === SoftSensorAI Additions Below ===" >> "$temp_merged"
